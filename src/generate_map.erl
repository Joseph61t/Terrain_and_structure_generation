...
make_map(Size_factor) ->
    ...
    % io:format("~p",[vary_map(Step,Size,Variance,gb_trees:update({Size,Size},lists:nth(rand:uniform(length(Vary_list)),Vary_list),Terrain_map4))]).
    save_map(1,string:concat("maps/Size_", integer_to_list(Size)),vary_map(Step,Size,Variance,gb_trees:update({Size,Size},lists:nth(rand:uniform(length(Vary_list)),Vary_list),Terrain_map4))).
    % Vary_list.
